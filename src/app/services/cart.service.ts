...
export class CartService {
  ...
  addToCart(ticketCategoryId: number, quantity: number = 1): void {
    debugger
    if (this.cart.has(ticketCategoryId)) {
      // Nếu sản phẩm đã có trong giỏ hàng, tăng số lượng lên `quantity`
      this.cart.set(ticketCategoryId, this.cart.get(ticketCategoryId)! + quantity);
    } else {
      // Nếu sản phẩm chưa có trong giỏ hàng, thêm sản phẩm vào với số lượng là `quantity`
      this.cart.set(ticketCategoryId, quantity);
    }
